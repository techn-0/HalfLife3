...
namespace _02_Scripts.Http.Components
{
    public sealed class GithubClient
    {
        ...
        public Awaitable<ApiResponse<Issue[]>> ListIssuesAsync(
            string owner, string repo, string state, CancellationToken ct)
        {
        ...
        public Awaitable<ApiResponse<Issue>> CreateIssueAsync(
            string owner, string repo, NewIssue body, CancellationToken ct)
        {
            var req = new ApiRequest(
        ...
        // GET /repos/{owner}/{repo}/commits (raw, per_page=1로 Link 헤더만 볼 용도)
        public Awaitable<ApiResponse<string>> ListCommitsRawAsync(
            string owner, string repo,
            DateTimeOffset? sinceUtc, DateTimeOffset? untilUtc,
            string author, int perPage, CancellationToken ct)
        {
